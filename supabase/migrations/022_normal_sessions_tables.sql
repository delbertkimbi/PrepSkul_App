...
CREATE TABLE IF NOT EXISTS public.session_payments (
  ...
  session_id UUID, -- References individual_sessions(id) - will add FK after table exists (nullable for now)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'individual_sessions' AND table_schema = 'public') THEN
    ...
    IF NOT EXISTS (
      ...
      -- Only set NOT NULL if column doesn't have NULL values
      -- Check if there are any NULL values first
      IF NOT EXISTS (
        SELECT 1 FROM public.session_payments WHERE session_id IS NULL
      ) THEN
        ALTER TABLE public.session_payments
        ALTER COLUMN session_id SET NOT NULL;
      END IF;
      
      -- Add FK constraint (will work even if column is nullable)
      ALTER TABLE public.session_payments
      ADD CONSTRAINT session_payments_session_id_fkey
      FOREIGN KEY (session_id) REFERENCES public.individual_sessions(id) ON DELETE CASCADE;
    END IF;
    ...
    IF NOT EXISTS (
      ...
      -- Only set NOT NULL if column doesn't have NULL values
      IF NOT EXISTS (
        SELECT 1 FROM public.session_feedback WHERE session_id IS NULL
      ) THEN
    ...
    IF NOT EXISTS (
      ...
      -- Only set NOT NULL if column doesn't have NULL values
      IF NOT EXISTS (
        SELECT 1 FROM public.session_attendance WHERE session_id IS NULL
      ) THEN
    ...
    IF NOT EXISTS (
      ...
      -- Only set NOT NULL if column doesn't have NULL values
      IF NOT EXISTS (
        SELECT 1 FROM public.tutor_earnings WHERE session_id IS NULL
      ) THEN
...
DO $$
BEGIN
  -- Only create policy if individual_sessions exists
  IF EXISTS (
...
BEGIN
  ...
  ELSE
    -- Create policy without individual_sessions check (only user_id check)
    EXECUTE '
    CREATE POLICY "Users can view their session attendance" ON public.session_attendance
      FOR SELECT
...
-- Note: COMMENT ON SCHEMA may fail if schema already has a comment
-- This is safe to ignore if it fails
DO $$
BEGIN
  COMMENT ON SCHEMA public IS 'Normal recurring sessions tables for payments, feedback, attendance, and earnings';
EXCEPTION
  WHEN OTHERS THEN
    -- Ignore if comment already exists or other error
    NULL;
END $$;
