-- STEP 1: FIX THE USER_TYPE CONSTRAINT FIRST
-- This removes the old constraint and adds the correct one
...
-- STEP 3: Delete any existing test data
...
DELETE FROM public.tutor_profiles WHERE user_id IN (SELECT id FROM public.profiles WHERE email LIKE '%@test.com');
DELETE FROM public.learner_profiles WHERE user_id IN (SELECT id FROM public.profiles WHERE email LIKE '%@test.com');
DELETE FROM public.profiles WHERE email LIKE '%@test.com';
DELETE FROM auth.users WHERE email LIKE '%@test.com';

-- STEP 4: Create test users in auth.users  
...
-- STEP 5: Create profiles with CORRECT user_type
...
-- SUCCESS! Test data created with fixed constraint
-- You now have:
