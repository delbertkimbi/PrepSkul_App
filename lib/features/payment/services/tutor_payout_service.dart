...
class TutorPayoutService {
  ...
  static Future<void> processPayout({
    ...
    try {
      ...
      // Process via Fapshi disbursement API via Next.js endpoint
      try {
        final apiBaseUrl = _getApiBaseUrl();
        final apiUrl = '$apiBaseUrl/payouts/process';
        ...
        LogService.info('📤 Processing payout via API: $apiUrl');
        ...
        final response = await http.post(
          ...
          onTimeout: () {
            throw Exception('Payout processing request timed out. Please try again.');
          },
        );

        if (response.statusCode == 200) {
          final responseData = jsonDecode(response.body) as Map<String, dynamic>;
          LogService.success('✅ Payout processing initiated: ${responseData['data']?['transId']}');
        } else {
          final errorData = jsonDecode(response.body) as Map<String, dynamic>;
          final errorMessage = errorData['error'] as String? ?? 'Failed to process payout';
          throw Exception(errorMessage);
        }
      } catch (e) {
        LogService.error('Error processing payout via API: $e');
        ...
        await _supabase
            ...
            .update({
              ...
              'admin_notes': 'API error: ${e.toString()}',
              'updated_at': DateTime.now().toIso8601String(),
            })
            .eq('id', payoutRequestId);
  ...
  /// Get API base URL for payout processing
