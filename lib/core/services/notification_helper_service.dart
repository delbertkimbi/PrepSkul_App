...
class NotificationHelperService {
  ...
  static Future<void> _sendNotificationViaAPI({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        LogService.warning(
          'Notification API returned status ${response.statusCode}: ${response.body}',
        );
        // Log error but don't throw - in-app notification already created
        if (response.statusCode == 429) {
          LogService.warning(
            'Rate limit detected for notification API. Email may not have been sent.',
          );
        } else if (response.statusCode >= 500) {
          LogService.warning(
            'Server error in notification API. Email may not have been sent.',
          );
        }
      }
      ...
      if (e is TimeoutException) {
        LogService.warning(
          'Notification API request timed out. In-app notification still created.',
        );
      } else {
        LogService.warning(
          'Notification API call failed: $e. In-app notification still created.',
        );
      }
  ...
  static Future<void> notifyBookingRequestCreated({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$studentName wants to book sessions for $subject. Review and respond to the request.',
      priority: 'high',
      ...
      metadata: {
        ...
        'sender_initials': studentName.isNotEmpty
            ? studentName[0].toUpperCase()
            : null,
      },
      sendEmail: true,
      sendPush:
          true, // Explicitly enable push notifications for booking requests
    );
  }
  ...
  static Future<void> notifyBookingRequestAccepted({
    ...
    final actionText = paymentRequestId != null ? 'Pay Now' : 'View Booking';
    ...
    await _sendNotificationViaAPI(
      ...
      metadata: {
        ...
        if (senderAvatarUrl != null) 'sender_avatar_url': senderAvatarUrl,
        'sender_initials': tutorName.isNotEmpty
  ...
  static Future<void> notifyMultiLearnerBookingAccepted({
    ...
    if (declinedLearners != null && declinedLearners.isNotEmpty) {
      ...
      message =
          '$tutorName has accepted your booking for: $acceptedList. '
      ...
      final acceptedList = acceptedLearners.join(', ');
      message =
          '$tutorName has accepted your booking request for: $acceptedList. '
  ...
  static Future<void> notifyTrialRequestAccepted({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your trial request with $tutorName has been approved. Tap to view details and pay.',
      priority: 'high',
  ...
  static Future<void> notifyPaymentFailed({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your payment of $amount $currency failed.${errorMessage != null ? ' Reason: $errorMessage' : ''} Please try again.',
      priority: 'high',
  ...
  static Future<void> notifyPaymentSuccessful({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your payment of $amount $currency was successful.${sessionType != null ? ' Your ${sessionType} session is confirmed!' : ''}',
      priority: 'normal',
      actionUrl: '/payments/$paymentId',
  ...
  static Future<void> schedulePaymentReminders({
    ...
    Future<void> scheduleLocalIfCurrentUser() async {
      final currentUserId = SupabaseService.client.auth.currentUser?.id;
      if (currentUserId == null || currentUserId != studentId) return;

      int _stableReminderId(String sessionId, String suffix) {
        final seed = '$sessionId-$suffix';
        var hash = 0;
      ...
      if (twoDaysBefore.isAfter(DateTime.now())) {
        await PushNotificationService().scheduleLocalOneTimeReminder(
          id: _stableReminderId(sessionId, '2d'),
          title: 'Payment reminder',
          body:
              'Your $sessionType session for $subject payment is due in 2 days.',
          when: twoDaysBefore,
          payload: {
      ...
      if (oneDayBefore.isAfter(DateTime.now())) {
        await PushNotificationService().scheduleLocalOneTimeReminder(
          id: _stableReminderId(sessionId, '1d'),
          title: 'Payment due tomorrow',
          body:
              'Your $sessionType session for $subject payment is due tomorrow.',
          when: oneDayBefore,
          payload: {
            'type': 'payment_reminder',
            'actionUrl': '/payments/$sessionId',
      ...
      if (twoHoursBefore.isAfter(DateTime.now())) {
        await PushNotificationService().scheduleLocalOneTimeReminder(
          id: _stableReminderId(sessionId, '2h'),
          title: 'Payment due soon',
          body:
              'Your $sessionType session for $subject payment is due in 2 hours.',
          when: twoHoursBefore,
          urgent: true,
          payload: {
            'type': 'payment_reminder',
    ...
    try {
      ...
      if (response.statusCode == 200) {
        LogService.success(
          'Payment reminders scheduled for session: $sessionId',
        );
      } else {
        LogService.warning(
          'Failed to schedule payment reminders: ${response.statusCode}',
        );
      }
      await scheduleLocalIfCurrentUser();
    } catch (e) {
      LogService.error('Error scheduling payment reminders: $e');
      // Do not fail flows because of reminder scheduling issues.
      await scheduleLocalIfCurrentUser();
    }
  }
  ...
  static Future<void> notifyPaymentDue({
    ...
    switch (reminderType) {
      case '2_days':
        title = '⏰ Payment Reminder';
        message =
            'Your $sessionType session for $subject (${amount.toStringAsFixed(0)} $currency) payment is due in 2 days.';
        priority = 'normal';
        break;
      case '1_day':
        title = '⏰ Payment Due Tomorrow';
        message =
            'Your $sessionType session for $subject (${amount.toStringAsFixed(0)} $currency) payment is due tomorrow!';
        priority = 'high';
        break;
      case '2_hours':
        title = '🚨 Payment Due Soon!';
        message =
            'Your $sessionType session for $subject (${amount.toStringAsFixed(0)} $currency) payment is due in 2 hours!';
        priority = 'urgent';
        break;
      default:
        title = '⏰ Payment Reminder';
        message =
            'Your $sessionType session for $subject (${amount.toStringAsFixed(0)} $currency) payment is due soon.';
        priority = 'normal';
    }
  ...
  static Future<void> notifySessionExpiring({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your $sessionType session for $subject is scheduled for ${sessionStart.toString().split('.')[0]}. Payment is required to confirm your session.',
      priority: 'high',
      actionUrl: '/payments/$sessionId',
      actionText: 'Pay Now',
      icon: '⚠️',
  ...
  static Future<void> scheduleSessionReminders({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        LogService.success(
          'Session reminders scheduled for session: $sessionId (24h, 1h, 15min)',
        );
      } else {
        LogService.warning(
          'Failed to schedule session reminders: ${response.statusCode}',
        );
  ...
  static Future<void> scheduleFeedbackReminder({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        LogService.success(
          'Feedback reminder scheduled for session: $sessionId',
        );
      } else {
        LogService.warning(
          'Failed to schedule feedback reminder: ${response.statusCode}',
        );
        ...
        await NotificationService.createNotification(
          ...
          message:
              'Please provide feedback for your completed session. It helps your tutor improve!',
          priority: 'normal',
      ...
      try {
        ...
        LogService.warning(
          'Could not create fallback feedback reminder notification: $e2',
        );
      }
    }
  }
  ...
  static Future<void> notifySessionCompleted({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your $sessionType session with $otherPartyName for $subject has been completed. Please leave a review!',
      priority: 'normal',
  ...
  static Future<void> notifyProfileApproved({
    ...
    try {
      ...
      if (profile == null || profile['user_type'] != 'tutor') {
        LogService.warning(
          'Skipping tutor profile approval notification for non-tutor user: $tutorId (role: ${profile?['user_type']})',
        );
        return;
      }
    } catch (e) {
      LogService.warning(
        'Could not verify user role for profile approval notification: $e',
      );
    ...
    await NotificationService.createNotification(
      ...
      message:
          'Congratulations! Your tutor profile has been approved and is now live. Students can now book sessions with you!',
      priority: 'high',
  ...
  static Future<void> notifyProfileNeedsImprovement({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your tutor profile needs some updates. Please review the feedback and update your profile.',
      priority: 'high',
      actionUrl: '/tutor/profile/edit',
      actionText: 'Update Profile',
      icon: '📝',
      metadata: {'improvement_requests': improvementRequests},
      sendEmail: true,
    );
  }
  ...
  static Future<void> notifyProfileRejected({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your tutor profile application was not approved. Reason: $rejectionReason. You can update your profile and re-apply.',
      priority: 'high',
      actionUrl: '/tutor/profile/edit',
      actionText: 'Update Profile',
      icon: '⚠️',
  ...
  static Future<void> notifyPaymentRequestPaid({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your payment has been confirmed. Your sessions are now active! Tap to view booking.',
      priority: 'high',
      actionUrl: bookingRequestId != null
    ...
    await _sendNotificationViaAPI(
      ...
      priority: 'normal',
      actionUrl: bookingRequestId != null
          ? '/tutor/bookings/$bookingRequestId'
          : '/tutor/bookings',
      actionText: 'View Booking',
      icon: '💰',
      metadata: {
  ...
  static Future<void> notifyPaymentRequestFailed({
    ...
    await _sendNotificationViaAPI(
      ...
      actionText: 'Retry Payment',
      icon: '⚠️',
      metadata: {'payment_request_id': paymentRequestId, 'reason': reason},
      sendEmail: true,
    );
  }
  ...
  static Future<void> notifyTrialPaymentCompleted({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your trial session payment has been confirmed. You can join from My Sessions at session time.',
      priority: 'high',
  ...
  static Future<void> notifyTrialPaymentReceived({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'A student has paid for their trial session in $subject. The session is now scheduled.',
      priority: 'normal',
      actionUrl: '/tutor/trials/$trialSessionId',
      actionText: 'View Session',
      icon: '💰',
      metadata: {'trial_session_id': trialSessionId, 'subject': subject},
      sendEmail: false,
    );
  }
  ...
  static Future<void> notifyTrialPaymentFailed({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your trial session payment could not be processed. Reason: $reason. Please try again.',
      priority: 'high',
      actionUrl: '/trials/$trialSessionId',
      actionText: 'Retry Payment',
      icon: '⚠️',
  ...
  static Future<void> notifySessionsCreated({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$sessionCount session${sessionCount > 1 ? 's' : ''} have been created for your booking with $tutorName. Sessions are now visible in your Sessions tab!',
      priority: 'high',
      actionUrl: '/sessions',
  ...
  static Future<void> notifyTutorSessionsCreated({
    ...
    await _sendNotificationViaAPI(
      ...
      title: 'Sessions Created',
      message:
          '$sessionCount session${sessionCount > 1 ? 's' : ''} have been created for your booking with $studentName. Sessions are now visible in your Sessions tab!',
      priority: 'normal',
  ...
  static Future<void> notifyTutorRequestCreated({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$requesterName has submitted a new tutor request. Please review and find a suitable tutor.',
      priority: 'high',
      actionUrl: '/admin/tutor-requests/$requestId',
      actionText: 'Review Request',
      icon: '🎓',
      metadata: {'request_id': requestId, 'requester_name': requesterName},
      sendEmail: true,
    );
  }
  ...
  static Future<void> notifyAdminsAboutNewUserSignup({
    ...
    try {
      ...
      if (adminResponse.isEmpty) {
        LogService.warning(
          'No admin users found to notify about new user signup',
        );
        return;
      }
      ...
      final userTypeDisplay = userType == 'student'
          ...
          : 'Tutor';
      final message =
          '$userName ($userTypeDisplay) has just signed up.\n\nEmail: $userEmail';
      ...
      LogService.success(
        'Notified ${adminResponse.length} admin(s) about new user signup: $userName',
      );
    } catch (e) {
  ...
  static Future<void> notifyAdminsAboutSurveyCompletion({
    ...
    try {
      ...
      if (adminResponse.isEmpty) {
        LogService.warning(
          'No admin users found to notify about survey completion',
        );
        return;
      }
      ...
      String message =
          '$userName (${userType == 'student' ? 'Student' : 'Parent'}) has completed their survey.';
      ...
      if (surveyDetails != null) {
        ...
        if (surveyDetails['budget_min'] != null &&
            surveyDetails['budget_max'] != null) {
          details.add(
            'Budget: ${surveyDetails['budget_min']}-${surveyDetails['budget_max']} XAF',
          );
        }
      ...
      for (final admin in adminResponse as List) {
        ...
        await _sendNotificationViaAPI(
          ...
          priority: 'normal',
          actionUrl:
              '/admin/${userType == 'student' ? 'students' : 'parents'}/$userId',
          actionText: 'View Profile',
          icon: '📝',
          metadata: {
            'user_id': userId,
      ...
      LogService.success(
        'Notified ${adminResponse.length} admin(s) about survey completion for $userName',
      );
    } catch (e) {
  ...
  static Future<void> notifyAdminsAboutSessionSafetyAlert({
    ...
    try {
      ...
      LogService.success(
        'Notified ${adminResponse.length} admin(s) about session safety: $sessionId',
      );
    } catch (e) {
  ...
  static Future<void> notifyTrialRequestUpdated({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$studentName has updated their trial request for $subject. Please review the changes.',
      priority: 'normal',
      actionUrl: '/trials/$trialId',
      actionText: 'Review Request',
      icon: '🔄',
  ...
  static Future<void> notifyTrialSessionModified({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$studentName has modified the approved trial session for $subject. Reason: $modificationReason. Please review and approve again.',
      priority: 'high',
      actionUrl: '/trials/$trialId',
  ...
  static Future<void> notifyTutorModificationRequest({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$tutorName has requested to modify the trial session for $subject. Reason: $reason. Please review and accept or decline.',
      priority: 'high',
      actionUrl: '/trials/$trialId',
      actionText: 'Review Request',
      icon: '🔄',
  ...
  static Future<void> notifyModificationAccepted({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$studentName has accepted your modification request. The session has been updated.',
      priority: 'normal',
      actionUrl: '/trials/$trialId',
      actionText: 'View Session',
      icon: '',
  ...
  static Future<void> notifyLowCreditsBalance({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          'Your credits balance (${balance.toStringAsFixed(0)}) is below the threshold (${threshold.toStringAsFixed(0)}). Please top up to continue using services.',
      priority: 'normal',
      actionUrl: paymentRequestId != null
          ? '/payments/$paymentRequestId'
          : '/credits',
  ...
  static Future<void> notifyTutorRequestUpdated({
    ...
    await _sendNotificationViaAPI(
      ...
      message:
          '$requesterName has updated their tutor request. Please review the changes.',
      priority: 'normal',
      actionUrl: '/admin/tutor-requests/$requestId',
      actionText: 'Review Request',
      icon: '🔄',
  ...
  static Future<void> notifyTutorRequestStatusChanged({
    ...
    switch (newStatus.toLowerCase()) {
      case 'in_progress':
        ...
        message =
            'Your tutor request is now being processed by our team. We\'ll keep you updated!';
        priority = 'normal';
        icon = '🔄';
        break;
      case 'matched':
      ...
      case 'closed':
        ...
        message =
            'Your tutor request has been closed.${adminNotes != null ? " Note: $adminNotes" : ""}';
        priority = 'normal';
        icon = '✅';
        break;
      default:
        ...
        message =
            'Your tutor request status has been updated to: ${newStatus.replaceAll('_', ' ')}.';
        priority = 'normal';
        icon = '📋';
    }
